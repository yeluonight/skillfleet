...
- 📥 **Device adoption** — import a Skill discovered on a device into the repository in one click; from then on it's editable, versioned, and deployable back to other paths.
- 🔢 **Readable & current versions** — auto-incrementing v1/v2/v3 numbers, with a pinnable "current" version as the deploy default.
- 🕒 **Time dimension** — see when each path's files were last edited and when the matching repository version was published.
- 🔄 **Operable Updates page** — view the diff, adopt upstream, capture local edits, or re-check, right from the row.
- 🗺️ **Path-centric views** — Fleet / device pages toggle by tool or by path, so shared roots like `.agents` read clearly across tools.
...
The server and agent share the same service backend: Linux uses a systemd user service, Windows uses
a scheduled task at logon.

```bash
skillfleet-server start     # installs + starts the background service
skillfleet-server status    # bind address / data dir / DB size / whether setup is complete
skillfleet-server restart   # restart
skillfleet-server stop      # stop the service

# Keep the systemd user service alive after logout (user services need linger):
loginctl enable-linger "$USER"

journalctl --user -u skillfleet-server -f     # follow logs (default logging goes to stderr → journald)
```

macOS does not write a launchd plist automatically. To keep it running, create
`~/Library/LaunchAgents/com.skillfleet.server.plist` manually, point `ProgramArguments` at
`skillfleet-server -foreground`, and `launchctl load` it. The server's log level / format is
controlled by the `logging` section of config.yaml (not a command-line flag).

**Upgrading the server**: re-run the same install command — the script compares the installed
version, skips if identical, otherwise stops the service → replaces the binary → restarts it (see the
Agent install section below for the upgrade details; both ends use the same mechanism).
...
(override with `INSTALL_DIR` / `SKILLFLEET_VERSION`). **The server and agent upgrade the same way**:
re-run the same install command to upgrade — it compares the installed version, skips if identical,
otherwise stops the service → replaces the binary → restarts it. `curl|sh` / `irm|iex` pipeline
installs upgrade directly; only running the script manually in an interactive terminal asks for
confirmation. To upgrade the server, use the `SKILLFLEET_COMPONENT=server` command shown above.
