...
skillfleet-server          # foreground, listens on :47890, prints a setup code to stderr on first run
```
...
Keep the server running (Linux systemd user service):

```bash
skillfleet-server start     # writes the unit + starts the service
...
the installed version, skips if identical, otherwise (interactive confirm or `--yes`) stops the
service → replaces the binary → restarts the service.
...
The agent ships built-in service commands (Linux uses a systemd user service — the unit is written
and started for you, no hand-editing):

```bash
skillfleet-agent start      # installs + starts the systemd user service
...
For foreground debugging you can tune logging: `skillfleet-agent -log-level debug -log-format json -log-file /tmp/sf.log`.
