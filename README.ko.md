...
server를 상주시키기 (Linux는 systemd 사용자 서비스, Windows는 로그온 시 작업 스케줄러):

```bash
skillfleet-server start     # 백그라운드 서비스를 작성하고 시작
...
업그레이드합니다: 설치된 버전과 비교하여 동일하면 건너뛰고, 다르면 서비스 중지 → 바이너리 교체 →
서비스 재시작을 수행합니다. `curl|sh` / `irm|iex` 파이프 실행은 바로 업그레이드하며, 대화형
확인은 스크립트를 직접 실행할 때만 표시됩니다.
