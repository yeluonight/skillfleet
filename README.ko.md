...
- 📥 **기기 채택** — 기기에서 스캔한 Skill을 한 번에 저장소로 가져오기. 이후 편집·버전 관리·다른 경로로 배포 가능.
- 🔢 **읽기 쉬운 버전 & current** — v1/v2/v3 자동 증가, 배포 기본값이 되는 "current(주) 버전" 지정 가능.
- 🕒 **시간 차원** — 각 경로의 파일이 언제 수정되었고 해당 저장소 버전이 언제 게시되었는지 한눈에.
- 🔄 **조작 가능한 업데이트 페이지** — 행에서 바로 diff 보기·업스트림 채택·로컬 변경 capture·재확인.
- 🗺️ **경로 중심 뷰** — Fleet / 기기 페이지를 도구별 / 경로별로 전환. `.agents` 같은 공유 루트가 여러 도구에서 명확하게.
...
skillfleet-server          # 최초 실행은 포그라운드, :47890 에서 수신, stderr에 setup code 출력
```

`http://<host>:47890`을 열고 stderr의 setup code로 관리자 초기화를 완료합니다. Server는 별도
설정 없이 바로 동작합니다(기본 데이터 디렉터리 `~/.skillfleet/server`, config.yaml 불필요). setup 완료 후
`skillfleet-server`를 다시 실행하면 백그라운드 서비스로 시작됩니다. 디버깅 시에는
`skillfleet-server -foreground`로 포그라운드 실행할 수 있습니다.

Server와 Agent는 동일한 서비스 백엔드를 공유합니다: Linux는 systemd 사용자 서비스, Windows는
로그온 시 작업 스케줄러를 사용합니다.

```bash
skillfleet-server start     # 백그라운드 서비스를 설치하고 시작
skillfleet-server status    # 수신 주소 / 데이터 디렉터리 / DB 크기 / setup 완료 여부 표시
skillfleet-server restart   # 재시작
skillfleet-server stop      # 서비스 중지

# Linux: 로그아웃 후에도 systemd 사용자 서비스를 유지하기(사용자 서비스는 linger 필요):
loginctl enable-linger "$USER"

journalctl --user -u skillfleet-server -f     # Linux 로그 보기(기본 로그는 stderr → journald)
```

macOS는 launchd plist를 자동 생성하지 않습니다. 상주시키려면
`~/Library/LaunchAgents/com.skillfleet.server.plist`를 수동으로 만들고 `ProgramArguments`를
`skillfleet-server -foreground`로 지정한 뒤 `launchctl load` 하세요. Server의 로그 레벨 /
형식은 config.yaml의 `logging` 섹션으로 제어합니다(명령줄 플래그가 아님).

**Server 업그레이드**: 동일한 설치 명령을 다시 실행하면 됩니다 — 스크립트가 설치된 버전과
비교하여 동일하면 건너뛰고, 다르면 서비스 중지 → 바이너리 교체 → 재시작을 수행합니다(메커니즘은
아래 Agent 설치 절의 업그레이드 설명과 동일합니다).
...
설치합니다(`INSTALL_DIR` / `SKILLFLEET_VERSION`으로 재정의 가능). **Server와 Agent의 업그레이드
방식은 동일합니다**: 동일한 설치 명령을 다시 실행하면 업그레이드됩니다 — 설치된 버전과 비교하여
동일하면 건너뛰고, 다르면 서비스 중지 → 바이너리 교체 → 서비스 재시작을 수행합니다. `curl|sh` /
`irm|iex` 파이프 실행은 바로 업그레이드하며, 대화형 확인은 스크립트를 직접 실행할 때만
표시됩니다. Server를 업그레이드하려면 위의 `SKILLFLEET_COMPONENT=server` 명령을 사용하세요.
...
# 3) 백그라운드 서비스 시작 (하트비트 + 후보 roots 스캔 + inventory 보고 + 다운링크 작업 수신)
skillfleet-agent
```
...
Agent에는 서비스 관리 명령이 내장되어 있습니다(Linux는 systemd 사용자 서비스, Windows는 로그온 시
작업 스케줄러 사용). enroll 완료 후 `skillfleet-agent`를 실행하면 백그라운드 서비스로 시작됩니다.
디버깅 시에는 `skillfleet-agent -foreground`를 사용합니다:

```bash
skillfleet-agent start      # 백그라운드 서비스를 설치하고 시작
...
포그라운드 디버깅 시 로그를 조정할 수 있습니다: `skillfleet-agent -foreground -log-level debug -log-format json -log-file /tmp/sf.log`.
