...
skillfleet-server          # 前台运行，监听 :47890，首次启动在 stderr 打印 setup code
```

打开 `http://<host>:47890`，用 stderr 里的 setup code 完成管理员初始化。Server 开箱即跑
（默认数据目录 `~/.skillfleet/server`，无需 config.yaml）。

让 server 常驻（Linux systemd 用户服务）：

```bash
skillfleet-server start     # 自动写 unit + 起服务
...
Agent 内置守护命令（Linux 走 systemd 用户服务，自动写 unit 并起停，无需手写）：

```bash
skillfleet-agent start      # 安装并启动 systemd 用户服务
skillfleet-agent status     # 看服务状态（active / PID / unit 路径）
skillfleet-agent restart    # 重启
skillfleet-agent stop       # 停止

# 让服务在登出后仍存活（用户服务需开启 linger）：
loginctl enable-linger "$USER"

journalctl --user -u skillfleet-agent -f      # 看日志（默认日志走 stderr → journald）
```

前台调试时可调日志：`skillfleet-agent -log-level debug -log-format json -log-file /tmp/sf.log`。
