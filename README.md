...
- 📥 **设备采纳** —— 一键把设备上扫到的 Skill 导入仓库，从此可编辑、版本化、反向下发到其他路径。
- 🔢 **可读版本 & 主版本** —— 版本号自增 v1/v2/v3，可指定「主版本」作为部署默认。
- 🕒 **时间维度** —— 看清每个路径的文件何时改、对应仓库版本何时发布。
- 🔄 **可操作更新页** —— 直接查看 diff、采纳上游、capture 本地修改、重新检查。
- 🗺️ **路径中心视图** —— Fleet / 设备页可按工具或按路径切换，`.agents` 等共享路径在多工具间一目了然。
...
Server 与 Agent 共用同一套守护后端：Linux 走 systemd 用户服务，Windows 走任务计划程序登录自启。

```bash
skillfleet-server start     # 安装并启动后台服务
skillfleet-server status    # 看监听地址 / 数据目录 / DB 大小 / setup 是否完成
skillfleet-server restart   # 重启
skillfleet-server stop      # 停服务

# Linux：让 systemd 用户服务在登出后仍存活（用户服务需开启 linger）：
loginctl enable-linger "$USER"

journalctl --user -u skillfleet-server -f     # Linux 看日志（默认日志走 stderr → journald）
```

macOS 目前不自动写 launchd。需要常驻时可手动创建 `~/Library/LaunchAgents/com.skillfleet.server.plist`，
把 `ProgramArguments` 指向 `skillfleet-server -foreground`，再 `launchctl load` 加载。Server 的日志
级别 / 格式由 config.yaml 的 `logging` 段控制（不是命令行 flag）。

**升级 server**：重复运行同一条安装命令即可——脚本对比已装版本，相同则跳过，不同则自动
停服务 → 替换二进制 → 重启服务（详见下方 Agent 安装段的升级说明，两端机制一致）。
...
`SKILLFLEET_VERSION` 覆盖）。**server 和 agent 升级机制相同**：重复运行同一条安装命令即升级——
脚本对比已装版本，相同则跳过，不同则自动停服务 → 替换二进制 → 重启服务。`curl|sh` / `irm|iex`
管道模式会直接升级；只有手动在交互式终端运行 `sh scripts/install.sh` 时才会询问确认。升级 server
时把上面的命令换成 `SKILLFLEET_COMPONENT=server` 那条即可。
