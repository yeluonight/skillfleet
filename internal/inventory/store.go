		...
		for _, sk := range tool.Skills {
			...
			if _, err := tx.ExecContext(ctx, `
				INSERT INTO discovered_skills(
					...
					content_sha256, file_count, total_bytes, warnings_json, created_at, modified_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			`,
				...
				nullable(sk.ContentSHA256), sk.FileCount, sk.TotalBytes, warningsJSON, ms, nullableInt(sk.ModifiedAt),
			); err != nil {
...
// nullableInt maps 0 to SQL NULL (an absent timestamp), any other value
// through unchanged.
