...
type Deps struct {
	...
	// Adopter publishes uploaded skill files to POST /agent/upload as a new
	// registry version (device -> registry adoption). When nil that route
	// returns 503. The server wires an adapter over the registry store.
	Adopter SkillAdopter
}
...
func NewRouter(d Deps) http.Handler {
	...
	// Skill adoption (mgmt-refactor track A): the agent uploads a captured
	// skill's file bytes here. AuthenticateLarge raises the body cap for this
	// one route since the payload carries base64 file content.
	mux.Handle("POST /agent/upload", d.AuthenticateLarge(http.HandlerFunc(d.handleUpload)))
	return mux
}
