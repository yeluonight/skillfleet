...
// MaxAgentUploadBytes is the larger cap used only by the skill-upload
// route (/agent/upload), where the body carries a skill's real file
// bytes (base64-encoded) being adopted into the registry. It is scoped to
// that one route via AuthenticateLarge so the cheap routes keep the tight
// 1 MiB cap.
const MaxAgentUploadBytes = 64 << 20 // 64 MiB
...
func (d Deps) Authenticate(next http.Handler) http.Handler {
	return d.authenticateLimit(MaxAgentBodyBytes, next)
}

// AuthenticateLarge is Authenticate with the larger MaxAgentUploadBytes
// body cap. Used only for /agent/upload, whose body carries skill file
// bytes; every other route keeps the tight default cap.
func (d Deps) AuthenticateLarge(next http.Handler) http.Handler {
	return d.authenticateLimit(MaxAgentUploadBytes, next)
}

func (d Deps) authenticateLimit(maxBody int64, next http.Handler) http.Handler {
	skew := d.MaxClockSkew
	if skew == 0 {
...
//
// SQLITE_BUSY is retried a bounded number of times with short backoff.
// Under WAL + busy_timeout=5s (internal/db/db.go), concurrent writers
// queue briefly; a transient BUSY at the tail of the queue is a false
// failure that the next attempt clears. This mirrors the swallow-on-
// BUSY precedent in internal/session/session.go:121, but here we retry
// rather than drop because a replayed nonce MUST still be claimed to
// keep the replay guard sound. ctx cancellation aborts mid-sleep.
func insertNonce(ctx context.Context, db *sql.DB, deviceID, nonce string, now time.Time) error {
	...
	for i := 0; i < maxRetries; i++ {
		...
		// Transient write-lock contention: retry after a short backoff.
		// "busy" covers SQLITE_BUSY; "locked" covers SQLITE_LOCKED.
		if strings.Contains(msg, "busy") || strings.Contains(msg, "locked") {
			lastErr = err
			if err := sleepCtx(ctx, retrySleep); err != nil {
...
// sleepCtx sleeps for d but aborts early when ctx is cancelled. Returns
// the ctx error if it fired, nil otherwise.
