...
type Device struct {
	...
	// SkillCount / RootCount come from the device's latest inventory run
	// (优化改造 §3.5). Populated only by List; zero for a device with no
	// run, and zero on the Device returned by Get (which does not JOIN).
	SkillCount int
	RootCount  int
}
...
func List(ctx context.Context, db DBExec, limit int) ([]Device, error) {
	...
	// LEFT JOIN each device's latest inventory run for skill/root counts.
	// "latest" = the run whose id matches a correlated subquery ordering
	// by created_at DESC, id DESC LIMIT 1 — the id tie-breaker (REVIEW#7)
	// guarantees exactly one row even if two runs share a created_at,
	// avoiding a device appearing twice. COALESCE floors no-run devices
	// to 0/0. Scan is inlined (not scanDevice) so Get's column order is
	// untouched.
	rows, err := db.QueryContext(ctx, `
		SELECT d.id, d.name, d.hostname, d.os, d.arch, d.agent_version,
		       d.status, d.created_at, d.last_seen_at,
		       COALESCE(ir.skill_count, 0), COALESCE(ir.root_count, 0)
		  FROM devices d
		  LEFT JOIN inventory_runs ir
		         ON ir.id = (
		            SELECT id FROM inventory_runs
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&dev.ID, &dev.Name, &hostname, &os, &arch, &av,
			&dev.Status, &createdAt, &lastSeen,
			&dev.SkillCount, &dev.RootCount,
		); err != nil {
			return nil, fmt.Errorf("devices: list scan: %w", err)
		}
		dev.Hostname = hostname.String
		dev.OS = os.String
		dev.Arch = arch.String
		dev.AgentVersion = av.String
		dev.CreatedAt = time.UnixMilli(createdAt)
		if lastSeen.Valid {
			dev.LastSeenAt = time.UnixMilli(lastSeen.Int64)
		}
		out = append(out, dev)
	}
	return out, rows.Err()
