...
const (
	...
	// StateNotDeployed marks a (device, registered-root) cell where the skill
	// has NO discovered_skills row: the root is registered but the skill was
	// never installed there. It is NOT produced by Classify — Classify only
	// ever sees skills the agent actually found on disk, so it can return
	// clean / local_modified / untracked but never "absent". The fleet-status
	// handler synthesizes not_deployed rows in a second pass over registered
	// roots that lack a discovered row, so the detail page can show every
	// deployable path (and offer a one-click deploy) rather than only the
	// paths that already hold the skill.
	StateNotDeployed LocalState = "not_deployed"
)
