...
type DiscoveredSkill struct {
	...
	// ModifiedAt is the newest file mtime in the skill directory (from the
	// fingerprint scan). Zero when the directory is empty/unreadable. It
	// answers "when was this skill last edited on the device" — distinct
	// from ContentSHA256 (what changed) and the inventory run time (when we
	// looked).
