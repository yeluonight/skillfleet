...
type RootSpec struct {
	...
	Shared bool
	// Unconsumed marks a root the tool's own CLI does not read — e.g. a
	// codex ~/.codex/skills path SkillFleet manages but codex itself ignores.
	// It lets the UI warn the operator that deploying there won't reach the
	// tool, and the scanner report an "unknown" effective state rather than
	// falsely "on". Advisory only, like Shared; never a security gate.
	Unconsumed bool
}
...
func CandidateSpecsFromRootSpecs(specs []RootSpec) []CandidateRootSpec {
	...
	for _, spec := range specs {
		...
		out = append(out, CandidateRootSpec{Scope: spec.Scope, Tmpl: spec.Tmpl, Shared: spec.Shared, Unconsumed: spec.Unconsumed})
	}
	return out
}
