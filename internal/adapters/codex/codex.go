...
//
// codex_user_codex is an UNCONSUMED root: SkillFleet manages ~/.codex/skills,
// but Codex's CLI itself does not read it (it reads ~/.agents/skills,
// <project>/.agents/skills, /etc/codex/skills — see config.toml). It exists so
// an operator can adopt the path and manage skills there, but ScanSkills marks
// any skill found there as "unknown" + a warning rather than falsely "on", so
// the UI never claims Codex will use it.
var rootSpecs = []adapters.RootSpec{
	{IDBase: "codex_user", Scope: adapters.ScopeUser, Tmpl: "~/.agents/skills", Shared: true},
	{IDBase: "codex_user_codex", Scope: adapters.ScopeUser, Tmpl: "~/.codex/skills", Unconsumed: true},
...
func (a *Adapter) ScanSkills(sc adapters.ScanContext, root adapters.SkillRoot) ([]adapters.DiscoveredSkill, error) {
	...
	// The unconsumed ~/.codex/skills root is managed by SkillFleet but not
	// read by Codex's CLI, so the config-driven "on/available" decode above
	// would be a lie. Override every skill found there to "unknown" with a
	// warning, so the UI shows an honest "we manage it, Codex won't use it".
	if root.ID == "codex_user_codex" {
		for i := range skills {
			skills[i].EffectiveState = adapters.StateUnknown
			skills[i].NativeState = "unconsumed_by_tool"
			skills[i].Warnings = append(skills[i].Warnings, adapters.Warning{
				Code:    "codex_unconsumed_root",
				Message: "Codex does not load skills from ~/.codex/skills; managed by SkillFleet only",
			})
		}
	}
	return skills, nil
