...
func TestAdapter_Identity(t *testing.T) {
	a := New()
	if a.Key() != "antigravity" || a.DisplayName() != "Antigravity" {
		t.Errorf("identity = %q / %q", a.Key(), a.DisplayName())
	}
}

func TestScanSkills_MatchesFixtureExpected(t *testing.T) {
	root, err := filepath.Abs(filepath.Join("..", "..", "..", "fixtures", "antigravity"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkillRoots_UserScope(t *testing.T) {
	...
	if len(roots) != 1 || roots[0].ID != "antigravity_user" || roots[0].Scope != adapters.ScopeUser {
