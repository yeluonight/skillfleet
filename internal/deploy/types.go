...
const (
	...
	// OpCaptureSkill asks the agent to read one discovered skill's real files
	// from a local root and upload the bytes, so the server can adopt them
	// into the registry (device -> registry, the inverse of install). It
	// addresses the source by Request.SkillName + Target{ToolKey, Scope,
	// RootID}; the agent resolves the root the same way install does, reads
	// the skill directory, and POSTs the files to /agent/upload. No plan and
	// no download — the data flows up, not down.
	OpCaptureSkill Operation = "capture_skill"
)

func (o Operation) valid() bool {
	switch o {
	case OpInstall, OpRollback, OpStateChange, OpRegisterRoot, OpRemoveRoot, OpCaptureSkill:
		return true
	}
	return false
}
...
type Request struct {
	...
	// Capture field: the absolute path of the discovered skill directory the
	// agent should read and upload (OpCaptureSkill). Like RootPath this is an
	// exception to "server never sends absolute paths" — the path comes from
	// the device's own inventory report (the skill_path it scanned), and the
	// agent re-validates it is under home before reading. Reuses SkillName for
	// the registry name and Target for provenance.
