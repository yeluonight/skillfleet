...
var (
	...
	ErrSkillExists   = errors.New("registry: skill already exists")
)
...
type Version struct {
	...
	Name          string
	VersionLabel  string
	VersionSeq    int // per-skill readable sequence (v1, v2, …); 0 if unset
	Kind          VersionKind
...
func (s *Store) PublishFromDir(ctx context.Context, srcDir string, p PublishParams, now time.Time) (Version, error) {
	...
	// One transaction for: compute the next per-skill version_seq, insert the
	// version row, and upsert the skills master row (pointing current_version
	// at the new version — "current follows latest"). SQLite serialises write
	// transactions, so two concurrent publishes of the same name cannot read
	// the same count and collide on version_seq.
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return Version{}, fmt.Errorf("registry: publish begin: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT MAX(version_seq) FROM skill_versions WHERE name = ?`, p.Name,
	).Scan(&maxSeq); err != nil {
		return Version{}, fmt.Errorf("registry: next version_seq: %w", err)
	}
	v.VersionSeq = int(maxSeq.Int64) + 1

	if _, err := tx.ExecContext(ctx, `
		INSERT INTO skill_versions(
			id, source_id, name, version_label, version_kind,
			source_commit, source_tag, source_release, base_version_id,
			content_sha256, manifest_json, package_path, created_at, version_seq)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`,
		...
		v.ContentSHA256, string(manifestJSON), v.PackagePath, now.UnixMilli(), v.VersionSeq,
	); err != nil {
		_ = os.Remove(filepath.Join(s.packagesDir, filepath.Base(relPath)))
		return Version{}, fmt.Errorf("registry: insert version: %w", err)
	}

	// Upsert the skills master row: create it on first publish, and point
	// current_version_id at this new version (current follows latest). An
	// operator can later pin a different current via SetCurrentVersion.
	if err := upsertSkillCurrent(ctx, tx, v.Name, v.ID, now); err != nil {
		_ = os.Remove(filepath.Join(s.packagesDir, filepath.Base(relPath)))
		return Version{}, fmt.Errorf("registry: upsert skill: %w", err)
	}

	if err := tx.Commit(); err != nil {
		_ = os.Remove(filepath.Join(s.packagesDir, filepath.Base(relPath)))
		return Version{}, fmt.Errorf("registry: publish commit: %w", err)
	}
	committed = true
...
// CurrentVersionID returns the skill's pinned current version id, or "" when
// none is pinned (the caller falls back to the newest version). An unknown
// skill name also returns "" (no error) — callers treat it as "use latest".
func (s *Store) CurrentVersionID(ctx context.Context, name string) (string, error) {
	var cur sql.NullString
	err := s.db.QueryRowContext(ctx,
		`SELECT current_version_id FROM skills WHERE name = ?`, name,
	).Scan(&cur)
	if errors.Is(err, sql.ErrNoRows) {
		return "", nil
	}
	if err != nil {
		return "", fmt.Errorf("registry: current version: %w", err)
	}
	return cur.String, nil
}

// SetCurrentVersion pins a skill's current (main) version. The version must
// exist and belong to the named skill; otherwise ErrVersionNotFnd. The skills
// row is created if absent (a skill that has versions but no master row yet,
// e.g. published before this table existed).
func (s *Store) SetCurrentVersion(ctx context.Context, name, versionID string, now time.Time) error {
	if name == "" {
		return ErrEmptyName
	}
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("registry: set current begin: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	// Validate the version exists and belongs to this skill.
	var vName string
	err = tx.QueryRowContext(ctx,
		`SELECT name FROM skill_versions WHERE id = ?`, versionID,
	).Scan(&vName)
	if errors.Is(err, sql.ErrNoRows) || (err == nil && vName != name) {
		return ErrVersionNotFnd
	}
	if err != nil {
		return fmt.Errorf("registry: set current lookup: %w", err)
	}

	if err := upsertSkillCurrent(ctx, tx, name, versionID, now); err != nil {
		return fmt.Errorf("registry: set current: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("registry: set current commit: %w", err)
	}
	committed = true
	return nil
}

// upsertSkillCurrent creates or updates a skill's master row, pointing
// current_version_id at versionID and stamping updated_at. Shared by
// PublishFromDir (current follows the new version) and SetCurrentVersion
// (operator pins a chosen version) so the upsert SQL lives in one place.
func upsertSkillCurrent(ctx context.Context, tx *sql.Tx, name, versionID string, now time.Time) error {
	...
	`, name, versionID, now.UnixMilli(), now.UnixMilli())
	return err
}

// DeleteSkill removes every trace of a skill name from the registry store:
// all of its versions, drafts (CASCADE clears skill_draft_files), and source
// binding. It does not delete on-disk .tgz archives; those are content-
// addressed and may be shared across names, while orphan archives are harmless
// and can be collected by a future GC command.
//
// Deleting a name with no rows is a no-op success so concurrent deletes do not
...
func (s *Store) DeleteSkill(ctx context.Context, name string) error {
	...
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("registry: delete skill begin: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM skill_versions WHERE name = ?`, name); err != nil {
		return fmt.Errorf("registry: delete skill versions: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM skill_drafts WHERE name = ?`, name); err != nil {
		return fmt.Errorf("registry: delete skill drafts: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM skill_sources WHERE name = ?`, name); err != nil {
		return fmt.Errorf("registry: delete skill sources: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM skills WHERE name = ?`, name); err != nil {
		return fmt.Errorf("registry: delete skill master: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("registry: delete skill commit: %w", err)
	}
	committed = true
	return nil
}

// RenameSkill changes a skill's name across all name-keyed registry tables in
// one transaction. The caller should pre-check newName for a clean client
// error; this method also checks inside the transaction so a concurrent create
// or rename cannot silently merge two skills' histories.
//
// newName == oldName is a no-op success. Both names must be non-empty.
func (s *Store) RenameSkill(ctx context.Context, oldName, newName string) error {
	if oldName == "" || newName == "" {
		return ErrEmptyName
	}
	if oldName == newName {
	...
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("registry: rename skill begin: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if err := tx.QueryRowContext(ctx, `
		SELECT 1
		 WHERE EXISTS(SELECT 1 FROM skill_versions WHERE name = ?)
		    OR EXISTS(SELECT 1 FROM skill_drafts WHERE name = ?)
	`, newName, newName).Scan(&collision); err == nil {
		return ErrSkillExists
	} else if !errors.Is(err, sql.ErrNoRows) {
		return fmt.Errorf("registry: rename collision check: %w", err)
	}

	if _, err := tx.ExecContext(ctx, `UPDATE skill_versions SET name = ? WHERE name = ?`, newName, oldName); err != nil {
		return fmt.Errorf("registry: rename skill versions: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `UPDATE skill_drafts SET name = ? WHERE name = ?`, newName, oldName); err != nil {
		return fmt.Errorf("registry: rename skill drafts: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `UPDATE skill_sources SET name = ? WHERE name = ?`, newName, oldName); err != nil {
		return fmt.Errorf("registry: rename skill sources: %w", err)
	}
	// Move the skills master row. Drop any stray row already at newName first
	// (the collision check above guarantees newName has no versions/drafts, so
	// a lingering master row would be an orphan), then rename oldName's.
	if _, err := tx.ExecContext(ctx, `DELETE FROM skills WHERE name = ?`, newName); err != nil {
		return fmt.Errorf("registry: rename skill master cleanup: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `UPDATE skills SET name = ? WHERE name = ?`, newName, oldName); err != nil {
		return fmt.Errorf("registry: rename skill master: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("registry: rename skill commit: %w", err)
	}
	committed = true
	return nil
}
...
func scanVersion(sc scanner) (Version, error) {
	var (
		...
		kind         string
		createdMS    int64
		versionSeq   sql.NullInt64
	)
	if err := sc.Scan(
		&v.ID, &sourceID, &v.Name, &versionLabel, &kind, &baseVersion,
		&v.ContentSHA256, &manifestJSON, &v.PackagePath, &createdMS, &versionSeq,
	); err != nil {
		return Version{}, err
	}
	...
	v.VersionSeq = int(versionSeq.Int64)
