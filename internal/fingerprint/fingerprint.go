...
type Fingerprint struct {
	...
	// ModTime is the newest file mtime in the tree (zero for an empty
	// tree). It is deliberately NOT part of Hash — the content digest must
	// stay mtime-insensitive (drift compares content, not timestamps) — but
	// it is surfaced so the inventory can answer "when was this last edited
	// on the device".
	ModTime time.Time
}
