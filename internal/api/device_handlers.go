...
func (d Deps) handleListDevices(w http.ResponseWriter, r *http.Request) {
	...
	for _, dev := range rows {
		out = append(out, deviceFromRow(dev))
	}
	writeJSON(w, http.StatusOK, map[string]any{"devices": out})
}

// handleGetDevice returns a single device view by id. 404 if absent.
// Read-only (auth, no CSRF). {id} is the device id.
...
func deviceFromRow(dev devices.Device) deviceView {
	v := deviceView{
		...
		SkillCount:   dev.SkillCount,
		RootCount:    dev.RootCount,
	}
