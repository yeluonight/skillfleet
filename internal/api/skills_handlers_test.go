...
// createSkillForTest publishes a bare skill via the create endpoint.
func createSkillForTest(t *testing.T, srv *httptest.Server, sc, cc *http.Cookie, name string) {
	t.Helper()
	req := newJSONReq(t, http.MethodPost, srv.URL+"/api/skills", map[string]string{"name": name})
	resp := authedDo(t, sc, cc, req)
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusCreated {
...
func TestDeleteSkill_Returns204(t *testing.T) {
	srv, d, reg := newTestServerWithRegistry(t)
	sc, cc := setupAndLogin(t, srv, d, "alice", "correcthorsebatterystaple")
	createSkillForTest(t, srv, sc, cc, "tmp")

	req, _ := http.NewRequest(http.MethodDelete, srv.URL+"/api/skills/tmp", nil)
	resp := authedDo(t, sc, cc, req)
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusNoContent {
		t.Fatalf("status = %d, want 204", resp.StatusCode)
	}
	// Skill is gone from the registry.
	exists, err := reg.SkillExists(context.Background(), "tmp")
	if err != nil {
		t.Fatal(err)
	}
	if exists {
...
func TestRenameSkill_Succeeds(t *testing.T) {
	srv, d, reg := newTestServerWithRegistry(t)
	sc, cc := setupAndLogin(t, srv, d, "alice", "correcthorsebatterystaple")
	createSkillForTest(t, srv, sc, cc, "old-name")

	req := newJSONReq(t, http.MethodPost, srv.URL+"/api/skills/old-name/rename",
		map[string]string{"new_name": "new-name"})
	resp := authedDo(t, sc, cc, req)
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
	...
	oldExists, _ := reg.SkillExists(context.Background(), "old-name")
	newExists, _ := reg.SkillExists(context.Background(), "new-name")
	if oldExists || !newExists {
		t.Errorf("after rename: oldExists=%v newExists=%v", oldExists, newExists)
	}
}

func TestRenameSkill_ConflictReturns409(t *testing.T) {
	srv, d, _ := newTestServerWithRegistry(t)
	sc, cc := setupAndLogin(t, srv, d, "alice", "correcthorsebatterystaple")
	createSkillForTest(t, srv, sc, cc, "skill-a")
	createSkillForTest(t, srv, sc, cc, "skill-b")

	req := newJSONReq(t, http.MethodPost, srv.URL+"/api/skills/skill-a/rename",
		map[string]string{"new_name": "skill-b"})
	resp := authedDo(t, sc, cc, req)
	defer resp.Body.Close()
...
func TestRenameSkill_SameNameIdempotent(t *testing.T) {
	srv, d, _ := newTestServerWithRegistry(t)
	sc, cc := setupAndLogin(t, srv, d, "alice", "correcthorsebatterystaple")
	createSkillForTest(t, srv, sc, cc, "stable")

	req := newJSONReq(t, http.MethodPost, srv.URL+"/api/skills/stable/rename",
		map[string]string{"new_name": "stable"})
	resp := authedDo(t, sc, cc, req)
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200", resp.StatusCode)
