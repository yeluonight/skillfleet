...
type versionView struct {
	...
	VersionSeq    int    `json:"version_seq,omitempty"`
...
func (d Deps) handleGetSkill(w http.ResponseWriter, r *http.Request) {
	...
	// current_version_id: the pinned current, or the newest version when
	// unpinned (versions are newest-first, so [0]). This is what deploy and
	// fork default to.
	if cur, err := d.Registry.CurrentVersionID(r.Context(), name); err != nil {
		d.logErr("skills: current version", err)
	} else if cur != "" {
		detail.CurrentVersionID = cur
	} else if len(views) > 0 {
		detail.CurrentVersionID = views[0].ID
	}
...
// handleDeleteSkill removes a skill entirely from the registry store
// (优化改造 §3.1). 204 on success regardless of whether any rows
// existed. The on-disk archives are left in place (orphan, harmless);
// the UI confirmation dialog tells the user device files are NOT
// uninstalled.
func (d Deps) handleDeleteSkill(w http.ResponseWriter, r *http.Request) {
	if !d.requireRegistry(w) {
		return
	}
	...
	if err := d.Registry.DeleteSkill(r.Context(), name); err != nil {
		d.logErr("skills: delete", err)
		writeError(w, http.StatusInternalServerError, "internal_error", "internal error")
		return
	}
	if d.Audit != nil {
...
// handleRenameSkill renames a skill (优化改造 §3.2). Request:
// {"new_name": "new-name"}. Validates new_name with the same rules as
// createSkill; 409 if newName is taken. Idempotent: new_name == name
// (after trim) returns 200 with the original name. On-disk archives
// keep their sha-names; only the DB rows move.
func (d Deps) handleRenameSkill(w http.ResponseWriter, r *http.Request) {
	if !d.requireRegistry(w) {
		return
	}
	...
	req, ok := decodeJSON[renameSkillRequest](w, r, 4*1024)
	if !ok {
		return
	}
	...
	if newName == oldName {
		// Idempotent no-op.
		writeJSON(w, http.StatusOK, renameSkillResponse{Name: oldName})
		return
	}
	// RenameSkill's in-tx collision check (skill_versions + skill_drafts) is
	// the authoritative, race-safe guard — no weaker pre-check needed here.
	if err := d.Registry.RenameSkill(r.Context(), oldName, newName); err != nil {
		if errors.Is(err, registry.ErrSkillExists) {
			writeError(w, http.StatusConflict, "already_exists", "a skill with that name already exists")
			return
		}
		d.logErr("skills: rename", err)
		writeError(w, http.StatusInternalServerError, "internal_error", "internal error")
		return
	}
	if d.Audit != nil {
		d.Audit.Write(r.Context(), audit.Record{
			...
			Target: audit.Target{Type: "skill", ID: newName},
			Detail: map[string]any{"from": oldName},
		})
	}
	writeJSON(w, http.StatusOK, renameSkillResponse{Name: newName})
}

type renameSkillRequest struct {
	NewName string `json:"new_name"`
}

type renameSkillResponse struct {
	Name string `json:"name"`
}
...
// handleSetCurrentVersion pins a skill's current (main) version — the one
// deploy defaults to. The version must belong to the named skill.
func (d Deps) handleSetCurrentVersion(w http.ResponseWriter, r *http.Request) {
	if !d.requireRegistry(w) {
		return
	}
	name := strings.TrimSpace(r.PathValue("name"))
	...
	req, ok := decodeJSON[setCurrentVersionRequest](w, r, 4*1024)
	if !ok {
		return
	}
	...
	if err := d.Registry.SetCurrentVersion(r.Context(), name, versionID, d.Now()); err != nil {
		if errors.Is(err, registry.ErrVersionNotFnd) {
			writeError(w, http.StatusNotFound, "not_found", "no such version for this skill")
			return
		}
		d.logErr("skills: set current version", err)
		writeError(w, http.StatusInternalServerError, "internal_error", "internal error")
		return
	}
	if d.Audit != nil {
		d.Audit.Write(r.Context(), audit.Record{
			...
			Target: audit.Target{Type: "skill", ID: name},
			Detail: map[string]any{"version_id": versionID},
		})
	}
	writeJSON(w, http.StatusOK, map[string]string{"current_version_id": versionID})
}
