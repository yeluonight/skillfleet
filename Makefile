...
# GO_TAGS pins purego so argon2 (and the other crypto asm) use the
# deterministic generic Go path. The amd64 SIMD assembly in
# golang.org/x/crypto/argon2 (blamkaSSE4) is non-deterministic on some
# CPUs — it computes a wrong result ~0.1% of the time — which surfaced as
# flaky login-401 tests AND a real ~0.1% chance of rejecting a correct
# production login. purego routes it through the generic Go path, which is
# deterministic. Control-plane traffic is low and TLS is fronted by a
# reverse proxy, so the perf cost is negligible.
GO_TAGS            ?= purego
GO_BUILD_FLAGS     ?= -trimpath -tags=$(GO_TAGS)
...
dev-server: ## Run the Go server with file watching (phase 1+).
	$(GO) run -tags=$(GO_TAGS) $(SERVER_PKG)

.PHONY: dev-agent
dev-agent: ## Run the Go agent against the local server (phase 2+).
	$(GO) run -tags=$(GO_TAGS) $(AGENT_PKG)
