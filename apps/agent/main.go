...
func runLoop(args []string) error {
	...
	logLevel := fl.String("log-level", "info", "log level: debug|info|warn|error")
	logFormat := fl.String("log-format", "text", "log format: text|json")
	logFile := fl.String("log-file", "", "log file path (default: stderr)")
	...
	if !*foreground && daemon.IsTerminal(os.Stdout) {
		bin, err := daemon.OwnBinaryPath()
		if err == nil {
			if err := daemon.Start(daemon.AgentSpec(bin, *configPath)); err == nil {
				fmt.Println("skillfleet-agent started in the background.")
				fmt.Println("Use `skillfleet-agent status` to check the service and `skillfleet-agent stop` to stop it.")
				return nil
			}
			fmt.Fprintln(os.Stderr, "skillfleet-agent: falling back to foreground mode")
		}
	}
	...
	log, err := serverlog.New(logWriter, *logLevel, *logFormat)
	if err != nil {
		return err
	}
	slog.SetDefault(log)
...
func printUsage(w *os.File) {
	...
	_, _ = fmt.Fprintln(w, "  skillfleet-agent                                    # start background service after enrolment")
	_, _ = fmt.Fprintln(w, "  skillfleet-agent -foreground                        # run loop in this terminal")
	...
	_, _ = fmt.Fprintln(w, "  skillfleet-agent start                              # install + start background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-agent stop                               # stop the background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-agent restart                            # restart the background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-agent status                             # show service status")
