...
export type Device = {
  ...
  created_at: number
  last_seen_at?: number
  // Counts from the device's latest inventory run (优化改造 §3.5); 0 when
  // the device has never reported a run.
  skill_count: number
  root_count: number
}
...
// v1.0 §8.2). Mirrors internal/drift.LocalState. "not_deployed" is special:
// it is never produced by drift.Classify (which only sees skills on disk),
// only synthesized by the fleet-status handler for a registered root the
// skill was never installed to.
export type LocalState = "clean" | "local_modified" | "untracked" | "not_deployed"

// FleetDeployment is one device×tool×scope install of a skill (优化改造
// §3.3). local_state / matched_version_id are computed by the backend's
// drift.Classify, not stored columns; matched_version_id is set only on
// clean rows.
export interface FleetDeployment {
  ...
  // Time dimension (mgmt-refactor track C), unix millis (0/absent unknown):
  // modified_at = when the device file was last edited; matched_version_
  // created_at = publish time of the registry version the device's content
  // matches (present only when local_state is clean).
  modified_at?: number
  matched_version_created_at?: number
}
...
// versionLabel is the short, human-readable version identifier: the readable
// sequence (v1, v2, …) when present, falling back to the raw id for legacy
// rows that predate version_seq. One definition so every version list /
// picker renders the same label.
export function versionLabel(v: SkillVersion): string {
  return v.version_seq ? `v${v.version_seq}` : v.id
}

export type SkillDetail = {
  name: string
  versions: SkillVersion[]
  // current/main version (mgmt-refactor track B). The pinned current, or the
  // newest version when unpinned — what deploy and fork default to.
...
// updateItemKey is the stable identity of an update row: upstream items are
// unique by (name, pending version), local-edit items by (name, device, tool,
// scope) — the same skill name can be modified on several devices. Shared by
// the card's React keys and the page's per-row busy marker so they agree.
...
// AdoptSkillBody addresses a discovered skill to adopt into the registry.
// tool_key/scope are optional disambiguators when the same skill name exists
// under more than one of the device's roots.
export type AdoptSkillBody = {
  tool_key?: string
  scope?: string
}
...
export type DeploymentJob = {
  id: string
  device_id: string
  operation: "install" | "rollback" | "state_change" | "register_root" | "remove_root" | "capture_skill"
...
export const api = {
  ...
  getDevice: (id: string) =>
    request<Device>("GET", `/api/devices/${encodeURIComponent(id)}`),
  ...
  adoptDeviceSkill: (id: string, name: string, body: AdoptSkillBody = {}) =>
    request<DeploymentJob>(
      "POST",
      `/api/devices/${encodeURIComponent(id)}/skills/${encodeURIComponent(name)}/adopt`,
      body,
    ),
  ...
  skillFleetStatus: (name: string) =>
    request<FleetStatus>("GET", `/api/skills/${encodeURIComponent(name)}/fleet-status`),
  renameSkill: (name: string, newName: string) =>
    request<{ name: string }>("POST", `/api/skills/${encodeURIComponent(name)}/rename`, {
      new_name: newName,
    }),
  deleteSkill: (name: string) =>
    request<void>("DELETE", `/api/skills/${encodeURIComponent(name)}`),
  setCurrentVersion: (name: string, versionId: string) =>
    request<{ current_version_id: string }>(
      "POST",
      `/api/skills/${encodeURIComponent(name)}/set-current-version`,
