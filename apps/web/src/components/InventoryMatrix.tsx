...
export function InventoryMatrix({
  ...
  // toolsBySkill maps a (root_path|skill_name) disk skill to the de-duplicated,
  // sorted list of tool_keys that read it. A skill in a shared root (e.g.
  // ~/.agents/skills) is scanned once per reading adapter, producing several
  // InventorySkill rows with different tool_keys — they all point at the same
  // files. The adopt action only needs one row (any row resolves the same
  // disk skill), but the operator should see *which* cli/ide read it before
  // adopting, so the adopt button surfaces this list. Falls back to skill_path
  // when an older agent didn't report root_path (same key as PathMasterDetail).
  const toolsBySkill = useMemo(() => {
    const sets = new Map<string, Set<string>>()
    for (const s of run?.skills ?? []) {
      const key = `${s.root_path ?? s.skill_path}|${s.name}`
      const set = sets.get(key)
      if (set) set.add(s.tool_key)
      else sets.set(key, new Set([s.tool_key]))
    }
    return new Map([...sets.entries()].map(([k, set]) => [k, [...set].sort()]))
  }, [run?.skills])
...
type SkillRowProps = {
  ...
  toolsBySkill: Map<string, string[]> // (root_path|name) -> tools reading that disk skill
...
function SkillTable({ skills, rowProps }: { skills: InventorySkill[]; rowProps: SkillRowProps }) {
  const { t } = useTranslation()
  const { showTool, driftByKey, toolsBySkill, registryNames, adopting, onAdopt, onChangeState } =
    rowProps
  return (
          ...
          <tr key={skillKey(s)} className="border-b last:border-0">
            ...
            <td className="px-3 py-1.5">
              {registryNames === null ? null : registryNames.has(s.name) ? (
                ...
                // A shared-root skill is one disk skill read by several tools;
                // show the full reader list so the operator knows which cli/ide
                // this adopt will cover before clicking. Single-reader skills
                // (the common case) stay quiet to avoid noise.
                const tools =
                  toolsBySkill.get(`${s.root_path ?? s.skill_path}|${s.name}`) ?? [s.tool_key]
                const context = tools.length > 1 ? tools.join(", ") : null
                return (
                  <div className="flex flex-col items-start gap-1">
                    {context ? (
                      <div
                        className="flex flex-wrap gap-1"
                        title={t("devices.adoptContext", { tools: context })}
                      >
                        {tools.map((tk) => (
                          <Badge key={tk} variant="secondary" className="px-1 py-0 text-[10px]">
                            {tk}
                    ...
                    <Button
                      ...
                      onClick={() => onAdopt(s)}
                      title={context ? `${t("devices.adoptHint")} · ${t("devices.adoptContext", { tools: context })}` : t("devices.adoptHint")}
                    >
