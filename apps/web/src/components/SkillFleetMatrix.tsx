...
import { api, type FleetDeployment } from "@/lib/api"
...
export function SkillFleetMatrix({ skillName }: { skillName: string }) {
  const { t } = useTranslation()
  const { data, error, refresh } = useApiResource(
    () => api.skillFleetStatus(skillName),
    { deps: [skillName], errorFallback: t("skills.err.loadFleet") },
  )
  const [changeTarget, setChangeTarget] = useState<FleetDeployment | null>(null)

  if (error) {
    return <p className="text-state-danger-600 text-sm">{error}</p>
  }
  if (!data) {
