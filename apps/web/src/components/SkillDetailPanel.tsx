...
import { Braces, FileCode, FilePlus, GitMerge, Minimize2, Save, Send, Trash2, Upload } from "lucide-react"
...
import { api, apiErrorMessage, versionLabel } from "@/lib/api"
...
export function VersionList({
  ...
  async function setCurrent(versionId: string) {
    ...
    setPinError(null)
    try {
      await api.setCurrentVersion(name, versionId)
      onChanged?.()
    } catch (err) {
      setPinError(apiErrorMessage(err, t("skills.setCurrentFailed")))
    } finally {
      setPinning(null)
    }
  }
  ...
  return (
    <div className="space-y-2">
      ...
      {pinError ? <p className="text-state-danger-600 text-xs">{pinError}</p> : null}
          ...
          return (
            <li key={v.id} className="text-sm">
              <div className="flex items-center gap-2">
                <button
                  className="flex flex-1 items-center justify-between gap-2 rounded px-2 py-1 hover:bg-muted/50"
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    {isCurrent ? (
                      <span className="bg-state-info-100 text-state-info-700 rounded px-1.5 py-0.5 text-[10px] font-medium">
                ...
                {!isCurrent ? (
                  <Button
                    ...
                    title={t("skills.setCurrentHint")}
                  >
                    {pinning === v.id ? t("skills.settingCurrent") : t("skills.setCurrent")}
                  </Button>
                ) : null}
              </div>
