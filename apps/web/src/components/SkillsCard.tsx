...
import { AlertCircle, FileText, Link2, Package, Plus, Upload } from "lucide-react"
...
function SkillList({ skills }: { skills: SkillSummary[] | null }) {
  const { t } = useTranslation()
      ...
      {skills.map((s) => (
        <li key={s.name} className="px-3 py-3">
          <button
            ...
            onClick={() => navigate("/skills/" + encodeURIComponent(s.name))}
          >
