...
func runDaemonStart(_ []string) error {
	...
	fmt.Println("skillfleet-server started (background service).")
...
func printUsage(w *os.File) {
	...
	_, _ = fmt.Fprintln(w, "  skillfleet-server                       # first run foreground for setup; later starts background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-server -foreground           # run server in this terminal")
	_, _ = fmt.Fprintln(w, "  skillfleet-server -config <path>        # run with a specific config")
	_, _ = fmt.Fprintln(w, "  skillfleet-server start                 # install + start background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-server stop                  # stop the background service")
	_, _ = fmt.Fprintln(w, "  skillfleet-server restart               # restart the background service")
