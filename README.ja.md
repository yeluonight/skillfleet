...
server を常駐させる（Linux は systemd ユーザーサービス、Windows はログオン時のタスクスケジューラ）：

```bash
skillfleet-server start     # バックグラウンドサービスを書き出して起動
...
異なればサービス停止 → バイナリ置換 → サービス再起動を行います。`curl|sh` / `irm|iex` の
パイプ実行ではそのままアップグレードし、対話確認はスクリプトを手動で実行した場合だけです。
