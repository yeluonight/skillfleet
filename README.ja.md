...
skillfleet-server          # フォアグラウンド実行、:47890 で待受、初回起動時に stderr へ setup code を出力
```

`http://<host>:47890` を開き、stderr の setup code で管理者初期化を完了します。Server は
そのまま動作します（デフォルトデータディレクトリ `~/.skillfleet/server`、config.yaml 不要）。

server を常駐させる（Linux systemd ユーザーサービス）：

```bash
skillfleet-server start     # unit を書き出してサービスを起動
...
インストールします（`INSTALL_DIR` / `SKILLFLEET_VERSION` で上書き可能）。再実行すると
その場でアップグレードします：インストール済みバージョンと比較し、同一ならスキップ、
異なれば（対話確認または `--yes`）サービス停止 → バイナリ置換 → サービス再起動を行います。
...
Agent にはサービス管理コマンドが組み込まれています（Linux は systemd ユーザーサービスを
使用 — unit の書き出しと起動は自動、手書き不要）：

```bash
skillfleet-agent start      # systemd ユーザーサービスをインストールして起動
...
フォアグラウンドでのデバッグ時はログを調整できます：`skillfleet-agent -log-level debug -log-format json -log-file /tmp/sf.log`。
