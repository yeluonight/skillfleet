...
- 📥 **デバイス取り込み** — デバイスでスキャンした Skill をワンクリックでリポジトリへ取り込み。以降は編集・バージョン管理・他パスへの配布が可能。
- 🔢 **可読バージョン & current** — v1/v2/v3 と自動採番、配布の既定にできる「current（主）バージョン」を指定可能。
- 🕒 **時間軸** — 各パスのファイルがいつ変更され、対応するリポジトリ版がいつ公開されたか一目で把握。
- 🔄 **操作できる更新ページ** — 行から直接 diff 表示・上流取り込み・ローカル変更の capture・再チェック。
- 🗺️ **パス中心ビュー** — Fleet / デバイスページをツール別 / パス別で切替。`.agents` など共有ルートが複数ツール間で明確に。
