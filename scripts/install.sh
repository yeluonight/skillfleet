...
# --yes (or SKILLFLEET_YES=1) skips the upgrade confirmation prompt. Useful
...
if [ -x "$dest" ]; then
  ...
  if [ "$YES" != "1" ]; then
    # Only prompt when we have a real terminal; piped installs default to no.
    if [ -t 0 ]; then
      ...
      echo "error: ${cli} ${old_version} is already installed; re-run with --yes (or SKILLFLEET_YES=1) to upgrade" >&2
      exit 1
    fi
  fi
  # If the old binary runs as a systemd user service, stop it so the file
  # isn't busy, and remember to restart after replacing.
  if command -v "$cli" >/dev/null 2>&1 && "$cli" status >/dev/null 2>&1; then
