...
if [ -x "$dest" ]; then
  ...
  # If the old binary runs as a systemd user service, stop it so the file
  # isn't busy and we can restart after replacing. Query systemctl directly
  # rather than `"$dest" status`/`stop`: those subcommands only exist in
  # v0.15.2+, and an older binary would treat `status` as a foreground run
  # and hang the installer forever waiting for it to exit.
  if [ "$os" = "linux" ] && command -v systemctl >/dev/null 2>&1 \
     && systemctl --user is-enabled --quiet "skillfleet-${COMPONENT}" 2>/dev/null; then
    systemctl --user stop "skillfleet-${COMPONENT}" >/dev/null 2>&1 || true
    replace_daemon=1
  fi
fi
