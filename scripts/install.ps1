...
try {
    ...
    $newVersion = Get-BinaryVersion $assetPath
    $restartTask = $false

    if (Test-Path $dest) {
        $oldVersion = Get-BinaryVersion $dest
        Write-Host "==> installed: $oldVersion  ->  new: $newVersion"
        if ($oldVersion -eq $newVersion -and $newVersion -ne "unknown") {
            Write-Host "==> already up to date"
            return
        }
        if (Test-SkillFleetTask) {
    ...
    if ($restartTask) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "==> upgraded and restarted skillfleet-$Component service"
        } else {
            Write-Warning "replaced skillfleet-$Component but failed to restart its service; run 'skillfleet-$Component start' manually"
        }
    }
